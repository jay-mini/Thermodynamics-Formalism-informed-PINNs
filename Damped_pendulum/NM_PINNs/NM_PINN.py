...
class NM_PINN(torch.nn.Module):
    def __init__(self, X_u_train, q_train, X_f_train, layers, lr, optimizer_name, iterations):
        ...
        self.iter = 0
        self.loss, self.loss_q, self.loss_f, self.lamb_tra, self.beta_tra = list(), list(), list(), list(), list()
    ...
    def net_f(self, t):
        ...
        f = q_tt + self.lamb * q_t + self.beta ** 2 * q
...
if __name__ == '__main__':
    ...
    q_train = np.array(q[::10]).reshape(-1, 1)
    print(q_train.shape)
    ...
    q_pred, w_pred, w_t_pred = model.predict(t.reshape(-1, 1))
    error_u_relative = np.linalg.norm(q.reshape(-1, 1) - q_pred.reshape(-1, 1), 2) / np.linalg.norm(
        q.reshape(-1, 1))

    pred_lamb = model.lamb.detach().cpu().numpy()
    pred_beta = model.beta.detach().cpu().numpy()
    R_pred = 0.5 * pred_lamb * w_pred ** 2 + pred_beta ** 2 * q_pred * w_pred + w_pred * w_t_pred
    S_pred = -0.5 * pred_beta ** 2 * q_pred ** 2 - 0.5 * w_pred ** 2
    ...
    print('Error u: %.5e, Error lamb: %.5e, Error beta: %.5e' % (error_u_relative, error_lamb, error_beta))
    ...
    axs[0].plot(t, R_pred, label='NM PINNs', color='darkorange', marker='*',
    ...
    axs[3].plot(q_pred, w_pred, marker='*', markersize=12,
