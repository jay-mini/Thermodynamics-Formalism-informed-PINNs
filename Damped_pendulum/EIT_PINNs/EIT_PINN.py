...
class EIT_PINN(torch.nn.Module):
    ...
    def net_S(self, q, w):
        return -0.5 * self.beta ** 2 * q ** 2 - 0.5 * w ** 2
    ...
    def net_f(self, t):
        ...
        w_t = -self.lamb * w - self.beta ** 2 * q

        S = self.net_S(q, w)

        S_u = torch.autograd.grad(
            S, q,
    ...
    def loss_pinn(self, verbose=True):
        ...
        if verbose:
            if self.iter % 100 == 0:
                print(
                    'epoch %d, gradient: %.5e, loss: %.5e, loss_u: %.5e, loss_f: %.5e' % (
                        self.iter, grad_norm, loss.item(), loss_u.item(), loss_f.item())
                )
            self.iter += 1
...
if __name__ == '__main__':
    ...
    model = EIT_PINN(t_u_train, u_train, t_f_train, layers, lr, optimizer_name, iterations)
    start_time = time.time()
    # loss = model.train()
    end_time = time.time()

    # torch.save(model.state_dict(), r'../Data/EIT_PINNs.pth')
    model.load_state_dict(torch.load(f'../Data/EIT_PINNs.pth'))
    ...
    axs[0].plot(t, R_pred, label='EIT PINNs', color='darkorange', marker='*',
